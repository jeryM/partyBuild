...
(function( window, undefined ) {
    "use strict";
    var auiTab = function(params,callback) {//干嘛的??
    	this.extend(this.params, params);//参数合并
        this._init(callback);
    };
    var tabItems;
    auiTab.prototype = {
        ...
        _init: function(callback) {
        	...
        	if(tabItems){
        		...
        		for(var i=0; i<tabItems.length; i++){
        			...
        			tabItems[i].setAttribute("data-item-order",i);//为当前对象设置位置序号
        			tabItems[i].onclick = function(e){//*****为每个菜单添加onclick事件,主要是改变了样式和返回当前index*****
                        if(!self.params.repeatClick){//是否允许重复点击
